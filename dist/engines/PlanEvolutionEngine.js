...
class PlanEvolutionEngine {
    static async evolvePlan(planId) {
        ...
        if (existingNextPlan) {
            console.log(`⚠️ Next plan already exists for plan ${planId}, returning existing.`);
        ...
        const lastInput = currentPlan.planJson.input; // The input used for the *current* plan
        // 2. Determine Mutations
        const nextInput = this.applyMutations(lastInput, blockEvaluation, profile);
        // 3. Generate New Plan
        const { planJson } = await PlanCompiler_1.PlanCompiler.generate(nextInput);
        // 4. Save New Plan
    ...
    static applyMutations(lastInput, evaluation, profile) {
        ...
        else if (actions.includes("DECREASE_VOLUME") ||
...
exports.PlanEvolutionEngine = PlanEvolutionEngine;
//# sourceMappingURL=PlanEvolutionEngine.js.map