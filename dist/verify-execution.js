...
async function main() {
    ...
    const plannedData = sessionToComplete.planned;
    const firstExercise = plannedData.exercises[0]; // { exerciseId, sets, ... }
    if (!firstExercise) {
        console.warn("   No exercises in first session, skipping completion logic check.");
    }
    else {
        ...
        await SessionService_1.SessionService.completeSession(user.id, {
            planId: plan.id,
...
//# sourceMappingURL=verify-execution.js.map