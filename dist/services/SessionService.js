...
const BlockEvaluationService_1 = require("./BlockEvaluationService");
...
class SessionService {
     ...
     */
    static async completeSession(userId, input) {
        console.log(`💪 SessionService: Completing session for plan ${input.planId}, Week ${input.week}, Day ${input.day}...`);
        ...
        if (!sessionRecord) {
            throw new Error(`Session not found for Plan ${input.planId}, W${input.week}D${input.day}`);
        }
        ...
        const updatedSession = await prisma.$transaction(async (tx) => {
            ...
            let allRpes = [];
            const plannedData = sessionRecord.planned;
            const plannedExercises = plannedData.exercises || [];
            for (const actualEx of input.exercises) {
                const plannedEx = plannedExercises.find((p) => p.exerciseId === actualEx.exerciseId);
                ...
                // Volume Calculation
                const actualExVolume = actualEx.sets.reduce((sum, s) => sum + s.reps * s.weight, 0);
                ...
                if (plannedEx) {
                    ...
                    const pReps = typeof plannedEx.reps === "number" ? plannedEx.reps : 10;
                ...
                await tx.userExerciseLog.create({
                    data: {
                        ...
                        equipmentUsed: actualEx.equipmentUsed,
            ...
            const avgRPE = allRpes.length > 0
                ...
                : null;
            const performanceScore = totalPlannedVolume > 0 ? totalActualVolume / totalPlannedVolume : 1.0;
            ...
            await ActivityService_1.ActivityService.recordActivity(userId, totalTimeSec, performanceScore, tx);
        ...
        if (isLast) {
            console.log(`🎉 Final session completed for plan ${input.planId}! Triggering auto-evolution...`);
     ...
     */
    static async isLastSession(planId, completedSessionId) {
        const allSessions = await prisma.userSession.findMany({
            where: { planId },
            select: { id: true, completedStatus: true },
        });
        // Check if all sessions are completed
        const allCompleted = allSessions.every((s) => s.completedStatus || s.id === completedSessionId);
        return allCompleted;
    }
    /**
     ...
     */
    static async triggerAutoEvolution(planId, userId) {
        try {
            console.log(`🔄 Auto-evolution triggered for plan ${planId} after final session completion...`);
            // 1. Evaluate the completed training block
            const evaluation = await BlockEvaluationService_1.BlockEvaluationService.evaluateBlock(planId);
            console.log(`✅ Block evaluation complete. Actions: ${JSON.stringify(evaluation.actions)}`);
            // 2. Generate the next personalized plan
            const newPlan = await PlanEvolutionEngine_1.PlanEvolutionEngine.evolvePlan(planId);
            console.log(`✅ Next plan generated automatically! Plan ID: ${newPlan.id}`);
            ...
            console.error(`❌ Auto-evolution failed for plan ${planId}:`, error instanceof Error ? error.message : error);
