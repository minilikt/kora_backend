...
async function main() {
  ...
  // 2. Run Compiler
  try {
    const plan = await PlanCompiler.generate({
      userId: user.id,
      days: 3,
      goal: "HYPERTROPHY",
      level: "INTERMEDIATE",
    ...
    // Check Day 1
    const week1 = (planJson.weeks as any[])[0];
    const day1 = (week1.sessions as any[]).find((s: any) => s.day === 1);

    if (day1) {
      console.log(`Day 1 Exercises: ${day1.exercises?.length || 0}`);
      (day1.exercises || []).forEach((ex: any) => {
        console.log(
          `- ${ex.exerciseName} (${ex.sets} sets) @ ${(ex.intensity || 0) * 100}% intensity`,
        );
      });
    } else {
